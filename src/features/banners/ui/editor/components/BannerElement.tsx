...
interface BannerElementProps {
  ...
  onStyleChange: (
    id: string,
    style: Partial<BannerElementData["style"]>
  ) => void;
...
export const BannerElement: React.FC<BannerElementProps> = ({
  ...
  const handleDragEnd = useCallback(
    (e: any, info: any) => {
      const newX = Math.max(
        0,
        Math.min(element.position.x + info.offset.x, containerSize.width - 50)
      );
      const newY = Math.max(
        0,
        Math.min(element.position.y + info.offset.y, containerSize.height - 20)
      );
  ...
  return (
    <motion.div
      ref={elementRef}
      className={`absolute cursor-move ${
        isSelected ? "ring-2 ring-primary rounded px-1" : ""
      }`}
      style={{
        left: element.position.x,
        top: element.position.y,
