...
import { GSAPPreset, GSAPAnimationConfig, splitTextToChars } from "./gsapAnimations";
...
function generateAnimationScript(config: GSAPAnimationConfig, hasSubtext: boolean): string {
  ...
  const animations: Record<string, string> = {
    ...
    "elastic-bounce": `
      function animateElement(el, delay) {
        const fromVars = { opacity: 0, scale: 0.3, ${direction === "up" || direction === "down" ? "y" : "x"}: ${direction === "up" || direction === "left" ? 100 : -100} };
