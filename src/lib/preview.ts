...
export const generatePreview = async (element: HTMLElement): Promise<string> => {
  try {
    ...
    if (element.offsetWidth === 0 || element.offsetHeight === 0) {
      console.warn("Attempted to capture a zero-sized element.");
      return ""; // Return empty string if not capturable
    }
    const dataUrl = await toPng(element, { 
        cacheBust: true, // Tweak for iframe content
        pixelRatio: 1, // Keep preview size smaller
     });
    return dataUrl;
  } catch (error) {
