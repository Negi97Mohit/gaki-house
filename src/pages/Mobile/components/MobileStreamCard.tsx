...
import { StreamChannel, formatViewerCount, PLATFORM_META } from "@/pages/platform/data/mockData";
...
export const MobileStreamCard: React.FC<MobileStreamCardProps> = ({ channel, variant = "default" }) => {
    ...
    if (variant === "compact") {
        return (
            <Link
                ...
                className="flex gap-3 p-2 rounded-xl hover:bg-muted/30 active:bg-muted/50 transition-colors active:scale-[0.98]"
            >
                {/* Compact thumbnail */}
                ...
                <div className="flex-1 min-w-0 py-0.5">
                    <p className="text-[13px] text-foreground font-semibold line-clamp-2 leading-tight">{channel.title}</p>
                    <p className="text-[11px] text-muted-foreground mt-1 truncate">{channel.displayName}</p>
                    <p className="text-[11px] text-muted-foreground/70 truncate">{channel.category}</p>
                </div>
            </Link>
        );
    }
