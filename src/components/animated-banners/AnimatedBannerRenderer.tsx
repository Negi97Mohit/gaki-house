  ...
  return (
    ...
    >
            ...
            <div className="flex gap-2">
              {data.links.slice(0, design.maxLinks).map((link, i) => (
                <motion.a
                  ...
                  style={{
                    width: socialState.style.fontSize || 32,
                    height: socialState.style.fontSize || 32,
                  }}
      ...
      <AnimatePresence>
        {isEditing &&
          textEditingId &&
          (textEditingId === "name" || textEditingId === "tagline") && (
            <BannerTextToolbar
              fontSize={getElementState(textEditingId)?.style.fontSize || 16}
              fontFamily={
                getElementState(textEditingId)?.style.fontFamily || "Inter"
              }
              color={getElementState(textEditingId)?.style.color || "#ffffff"}
              fontWeight={
                getElementState(textEditingId)?.style.fontWeight || "normal"
              }
              onFontSizeChange={(size) =>
                handleStyleChange(textEditingId, { fontSize: size })
              }
              onFontFamilyChange={(family) =>
                handleStyleChange(textEditingId, { fontFamily: family })
              }
              onColorChange={(color) =>
                handleStyleChange(textEditingId, { color })
              }
              onFontWeightChange={(weight) =>
                handleStyleChange(textEditingId, { fontWeight: weight })
              }
              position={{
