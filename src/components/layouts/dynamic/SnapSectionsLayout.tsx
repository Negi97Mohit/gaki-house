    ...
    >
      ...
      {sections.map((section, i) => (
        ...
        >
          ...
          {/* Overlay Text */}
          <div className="relative z-10 text-center mix-blend-difference w-full px-8">
            <input
              value={props.layout.customSectionData?.[section.id]?.seq ?? `Story 0${i + 1}`}
              onChange={(e) => handleUpdateText(section.id, "seq", e.target.value)}
              className="bg-transparent border-none text-2xl tracking-[0.5em] uppercase mb-4 text-center w-full focus:outline-none"
              style={{ color: textColor }} // Actually mix-blend-difference inverts it, but let's set base
            />
            <input
              ...
              className="bg-transparent border-none text-8xl font-black uppercase text-center w-full focus:outline-none"
              style={{ color: textColor }}
            />
          </div>

          {/* Delete Button */}
          <div className={cn("absolute top-8 right-8 z-50 transition-opacity duration-200", hoveredSectionId === section.id ? "opacity-100" : "opacity-0")}>
            <button
