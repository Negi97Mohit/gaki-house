...
const createDefaultElements = (design: AnimatedBannerDesign): BannerElementData[] => {
  ...
  if (design.showAvatar) {
    elements.push({
      ...
      visible: true,
      position: { x: 20, y: 20 },
      style: { fontSize: 16, fontFamily: 'Inter', color: '#ffffff', fontWeight: 'normal' }
    });
  }
  ...
  elements.push({
    ...
    visible: true,
    position: { x: 80, y: 25 },
    style: { fontSize: 24, fontFamily: 'Inter', color: '#ffffff', fontWeight: 'bold' }
  });
...
export const EditableBannerContent: React.FC<EditableBannerContentProps> = ({
  ...
  const [secondaryColor, setSecondaryColor] = useState(
    initialState?.secondaryColor || design.particleSettings?.colorVariant || '#3b82f6'
  );
  ...
  const handleStyleChange = useCallback((id: string, style: Partial<BannerElementData['style']>) => {
    setElements(prev => prev.map(el => 
