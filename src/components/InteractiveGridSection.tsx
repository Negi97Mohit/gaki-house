...
export const InteractiveGridSection: React.FC<InteractiveGridSectionProps> = ({
    ...
    return (
        ...
        >
            ...
            <div className="absolute top-2 right-2 z-50 flex gap-2 opacity-0 group-hover/section:opacity-100 transition-opacity">
                <DropdownMenu>
            ...
            {sceneSize.width > 0 && (
                <>
                    ...
                    >
                        <div className="w-full h-full pointer-events-none overflow-hidden rounded-md">
                            <CameraRenderer
                                ...
                                onCameraDeviceChange={(deviceId) => onUpdate({ selectedDeviceId: deviceId })}
                                ...
                                onAutoFramingChange={(enabled) => onUpdate({ isAutoFramingEnabled: enabled })}
                                isBeautifyEnabled={settings.isBeautifyEnabled}
                                onBeautifyToggle={(enabled) => onUpdate({ isBeautifyEnabled: enabled })}
                                isLowLightEnabled={settings.isLowLightEnabled}
                                onLowLightToggle={(enabled) => onUpdate({ isLowLightEnabled: enabled })}
                                videoFilter={settings.videoFilter}
                                onVideoFilterChange={(filter) => onUpdate({ videoFilter: filter })}
                                isNeonEdgeEnabled={settings.isNeonEdgeEnabled}
                                onNeonEdgeToggle={(enabled) => onUpdate({ isNeonEdgeEnabled: enabled })}
                                neonIntensity={settings.neonIntensity}
                                onNeonIntensityChange={(value) => onUpdate({ neonIntensity: value })}
                                neonColor={settings.neonColor}
                                onNeonEdgeColorChange={(color) => onUpdate({ neonColor: color })}
                                zoomSensitivity={settings.zoomSensitivity}
                                onZoomSensitivityChange={(value) => onUpdate({ zoomSensitivity: value })}
                                trackingSpeed={settings.trackingSpeed}
                                onTrackingSpeedChange={(value) => onUpdate({ trackingSpeed: value })}
                                cameraBackground={settings.cameraBackground}
                                onCameraBackgroundChange={(bgId) => onUpdate({ cameraBackground: bgId })}
                                onCustomBackgroundUpload={(file) => {
                                    const url = URL.createObjectURL(file);
                                    onUpdate({ cameraBackground: "image", customBackgroundUrl: url });
                                }}
                                cameraAspectRatio={settings.cameraAspectRatio}
                                onCameraAspectRatioChange={(ratio) => onUpdate({ cameraAspectRatio: ratio })}
                                customAspectRatio={settings.customAspectRatio}
                                onCustomAspectRatioChange={(ratio) => onUpdate({ customAspectRatio: ratio })}
                                isFaceTrackingEnabled={settings.isFaceTrackingEnabled}
                                onFaceTrackingToggle={(enabled) => onUpdate({ isFaceTrackingEnabled: enabled })}
                                activeInteractiveFilter={settings.activeInteractiveFilter}
                                onInteractiveFilterChange={(filter) => onUpdate({ activeInteractiveFilter: filter })}
                                filterIntensity={settings.filterIntensity}
                                onFilterIntensityChange={(value) => onUpdate({ filterIntensity: value })}
                                filterColor={settings.filterColor}
                                onFilterColorChange={(color) => onUpdate({ filterColor: color })}
                                filterTarget={settings.filterTarget}
                                onFilterTargetChange={(target) => onUpdate({ filterTarget: target })}
                            />
                        </div>
                    </Rnd>
                    ...
                    {isCameraSelected && (
                        <PipControlsToolbar
                            position={{
                                x: (sceneSize.width * (settings.pipPosition?.x || 0)) / 100 + (sceneSize.width * (settings.pipSize?.width || 30)) / 200,
                                y: (sceneSize.height * (settings.pipPosition?.y || 0)) / 100,
                            }}
