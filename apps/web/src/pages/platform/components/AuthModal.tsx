  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/70 backdrop-blur-md" onClick={handleClose}>
      <div className="relative w-full max-w-sm mx-4 bg-card border border-border rounded-3xl shadow-2xl shadow-black/30 overflow-hidden animate-in fade-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
        ...
        <button
          onClick={handleClose}
          className="absolute top-3 right-3 p-1.5 rounded-full text-muted-foreground hover:text-foreground hover:bg-muted/80 transition-all z-10"
          aria-label="Close"
        >
          <X className="w-4 h-4" />
        </button>
        ...
        <div className="p-7 relative">
          ...
          >
            {googleLoading ? <Loader2 className="w-4 h-4 text-muted-foreground animate-spin" /> : (
            ...
            {googleLoading ? "Cancel Sign in" : "Continue with Google"}
          </button>
