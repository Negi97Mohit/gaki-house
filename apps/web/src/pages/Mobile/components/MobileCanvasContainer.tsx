...
import { MainCanvasArea } from "@/pages/Index/components/MainCanvasArea";
import { MobileStudioToolsDrawer } from "./MobileStudioToolsDrawer";
import {
  CaptionStyle,
  GeneratedOverlay,
  SceneState,
...
import { useCanvasAi } from "@/pages/Index/hooks/useCanvasAi";
import { useCanvasBanners } from "@/pages/Index/hooks/useCanvasBanners";
import { getAllPropsForScene } from "@/pages/Index/utils/canvasProps";
import { useLayoutManager } from "@/pages/Index/hooks/useLayoutManager";
...
export interface MobileCanvasContainerProps {
  layoutManager?: any; // optional — real hook is used internally
...
export const MobileCanvasContainer: React.FC<MobileCanvasContainerProps> = ({
  layoutManager: _externalLayoutManager,
  ...
  const {
    ...
    useShallow((state) => ({
      ...
      setCameraShape: state.setCameraShape,
      splitRatio: state.splitRatio,
      setSplitRatio: state.setSplitRatio,
      pipPosition: state.pipPosition,
      setPipPosition: state.setPipPosition,
      pipSize: state.pipSize,
      setPipSize: state.setPipSize,
  ...
  const {
    ...
    useShallow((state) => ({
      ...
      setCustomMaskUrl: state.setCustomMaskUrl,
      activeOverlays: state.activeOverlays,
      setActiveOverlays: state.setActiveOverlays,
      textOverlays: state.textOverlays,
      setTextOverlays: state.setTextOverlays,
      fileOverlays: state.fileOverlays,
      setFileOverlays: state.setFileOverlays,
      browserOverlays: state.browserOverlays,
      setBrowserOverlays: state.setBrowserOverlays,
      ...
      blankCanvasColor: state.blankCanvasColor,
      setBlankCanvasColor: state.setBlankCanvasColor,
      videoFilter: state.videoFilter,
      setVideoFilter: state.setVideoFilter,
      ...
      setSelectedBrowserId: state.setSelectedBrowserId,
      ...
      setSelectedGeneratedId: state.setSelectedGeneratedId,
      deselectAll: state.deselectAll,

      // Added State
      pipRotation: state.pipRotation,
      setPipRotation: state.setPipRotation,
      pipBorder: state.pipBorder,
      setPipBorder: state.setPipBorder,
      pipShadow: state.pipShadow,
      setPipShadow: state.setPipShadow,
      cameraAspectRatio: state.cameraAspectRatio,
      setCameraAspectRatio: state.setCameraAspectRatio,
      customAspectRatio: state.customAspectRatio,
      setCustomAspectRatio: state.setCustomAspectRatio,
      activeInteractiveFilter: state.activeInteractiveFilter,
      setActiveInteractiveFilter: state.setActiveInteractiveFilter,
      filterIntensity: state.filterIntensity,
      setFilterIntensity: state.setFilterIntensity,
      filterColor: state.filterColor,
      setFilterColor: state.setFilterColor,
      filterTarget: state.filterTarget,
      setFilterTarget: state.setFilterTarget,
      isAutoFramingEnabled: state.isAutoFramingEnabled,
      setIsAutoFramingEnabled: state.setIsAutoFramingEnabled,
      isBeautifyEnabled: state.isBeautifyEnabled,
      setIsBeautifyEnabled: state.setIsBeautifyEnabled,
      isLowLightEnabled: state.isLowLightEnabled,
      setIsLowLightEnabled: state.setIsLowLightEnabled,
      isNeonEdgeEnabled: state.isNeonEdgeEnabled,
      setIsNeonEdgeEnabled: state.setIsNeonEdgeEnabled,
      neonIntensity: state.neonIntensity,
      setNeonIntensity: state.setNeonIntensity,
      neonColor: state.neonColor,
      setNeonColor: state.setNeonColor,
      cameraBackground: state.cameraBackground,
      setCameraBackground: state.setCameraBackground,
      customBackgroundUrl: state.customBackgroundUrl,
      setCustomBackgroundUrl: state.setCustomBackgroundUrl,
      zoomSensitivity: state.zoomSensitivity,
      setZoomSensitivity: state.setZoomSensitivity,
      trackingSpeed: state.trackingSpeed,
      setTrackingSpeed: state.setTrackingSpeed,
      isFaceTrackingEnabled: state.isFaceTrackingEnabled,
      setIsFaceTrackingEnabled: state.setIsFaceTrackingEnabled,
  ...
  const {
    ...
    useShallow((state) => ({
      ...
      isFsSidebarOpen: state.isFsSidebarOpen,
      setFsSidebarOpen: state.setFsSidebarOpen,
  ...
  // Compatibility: updateActiveScene
  const updateActiveScene = useCallback(
    (updater: (scene: any) => any) => {
      const newScene = updater(activeScene);

      if (newScene.isAudioOn !== activeScene.isAudioOn)
        setAudioOn(newScene.isAudioOn ?? false);
      if (newScene.isVideoOn !== activeScene.isVideoOn)
        setVideoOn(newScene.isVideoOn ?? false);
      if (
        newScene.selectedAudioDevice !== activeScene.selectedAudioDevice &&
        newScene.selectedAudioDevice
      )
        setSelectedAudioDevice(newScene.selectedAudioDevice);
      if (
        newScene.selectedVideoDevice !== activeScene.selectedVideoDevice &&
      ...
      if (newScene.layoutMode !== activeScene.layoutMode)
        setLayoutMode(newScene.layoutMode ?? "solo");
      if (newScene.cameraShape !== activeScene.cameraShape)
        setCameraShape(newScene.cameraShape ?? "rectangle");
      if (newScene.splitRatio !== activeScene.splitRatio)
      ...
      if (newScene.activeOverlays !== activeScene.activeOverlays)
        setActiveOverlays(newScene.activeOverlays ?? []);
      if (newScene.textOverlays !== activeScene.textOverlays)
        setTextOverlays(newScene.textOverlays ?? []);
      if (newScene.fileOverlays !== activeScene.fileOverlays)
        setFileOverlays(newScene.fileOverlays ?? []);
      if (newScene.browserOverlays !== activeScene.browserOverlays)
        setBrowserOverlays(newScene.browserOverlays ?? []);
      if (newScene.canvasLayout !== activeScene.canvasLayout)
        setCanvasLayout(newScene.canvasLayout ?? null);
      if (newScene.backgroundEffect !== activeScene.backgroundEffect)
        setBackgroundEffect(newScene.backgroundEffect ?? "none");
      if (newScene.backgroundImageUrl !== activeScene.backgroundImageUrl)
        setBackgroundImageUrl(newScene.backgroundImageUrl ?? null);
      if (newScene.blankCanvasColor !== activeScene.blankCanvasColor)
        setBlankCanvasColor(newScene.blankCanvasColor ?? "#000000");
      if (newScene.videoFilter !== activeScene.videoFilter)
        setVideoFilter(newScene.videoFilter ?? "none");
      if (
        newScene.captionStyle !== activeScene.captionStyle &&
      ...
      if (newScene.captionsEnabled !== activeScene.captionsEnabled)
        setCaptionsEnabled(newScene.captionsEnabled ?? true);

      // New setters
      if (newScene.pipRotation !== activeScene.pipRotation)
        setPipRotation(newScene.pipRotation ?? 0);
      if (newScene.pipBorder !== activeScene.pipBorder)
        setPipBorder(newScene.pipBorder ?? { color: "#FFFFFF", width: 0 });
      if (newScene.pipShadow !== activeScene.pipShadow)
        setPipShadow(
          newScene.pipShadow ?? { blur: 0, color: "rgba(0,0,0,0.5)" }
        );
      if (newScene.cameraAspectRatio !== activeScene.cameraAspectRatio)
        setCameraAspectRatio(newScene.cameraAspectRatio ?? "16:9");
      if (newScene.customAspectRatio !== activeScene.customAspectRatio)
        setCustomAspectRatio(newScene.customAspectRatio ?? "");
      if (
        newScene.activeInteractiveFilter !== activeScene.activeInteractiveFilter
      )
        setActiveInteractiveFilter(newScene.activeInteractiveFilter ?? "none");
      if (newScene.filterIntensity !== activeScene.filterIntensity)
        setFilterIntensity(newScene.filterIntensity ?? 0.5);
      if (newScene.filterColor !== activeScene.filterColor)
        setFilterColor(newScene.filterColor ?? "#000000");
      if (newScene.filterTarget !== activeScene.filterTarget)
        setFilterTarget(newScene.filterTarget ?? "both");
      if (newScene.isAutoFramingEnabled !== activeScene.isAutoFramingEnabled)
        setIsAutoFramingEnabled(newScene.isAutoFramingEnabled ?? false);
      if (newScene.isBeautifyEnabled !== activeScene.isBeautifyEnabled)
        setIsBeautifyEnabled(newScene.isBeautifyEnabled ?? false);
      if (newScene.isLowLightEnabled !== activeScene.isLowLightEnabled)
        setIsLowLightEnabled(newScene.isLowLightEnabled ?? false);
      if (newScene.isNeonEdgeEnabled !== activeScene.isNeonEdgeEnabled)
        setIsNeonEdgeEnabled(newScene.isNeonEdgeEnabled ?? false);
      if (newScene.neonIntensity !== activeScene.neonIntensity)
        setNeonIntensity(newScene.neonIntensity ?? 50);
      if (newScene.neonColor !== activeScene.neonColor)
        setNeonColor(newScene.neonColor ?? "#00FFFF");
      if (newScene.cameraBackground !== activeScene.cameraBackground)
        setCameraBackground(newScene.cameraBackground ?? "none");
      if (newScene.customBackgroundUrl !== activeScene.customBackgroundUrl)
        setCustomBackgroundUrl(newScene.customBackgroundUrl ?? null);
      if (newScene.zoomSensitivity !== activeScene.zoomSensitivity)
        setZoomSensitivity(newScene.zoomSensitivity ?? 0.5);
      if (newScene.trackingSpeed !== activeScene.trackingSpeed)
        setTrackingSpeed(newScene.trackingSpeed ?? 0.5);
      if (newScene.isFaceTrackingEnabled !== activeScene.isFaceTrackingEnabled)
        setIsFaceTrackingEnabled(newScene.isFaceTrackingEnabled ?? false);
      if (newScene.canvasAspectRatio !== activeScene.canvasAspectRatio)
        setCanvasAspectRatio(newScene.canvasAspectRatio ?? "16:9");
    },
    [
      activeScene,
      setAudioOn,
    ...
    (key: string, value: any) => {
      switch (key) {
        ...
        case "pipSize":
          ...
          break;
        case "customMaskUrl":
          setCustomMaskUrl(value);
          break;
        case "activeOverlays":
          setActiveOverlays(value);
          break;
        case "textOverlays":
          setTextOverlays(value);
          break;
        case "canvasLayout":
          setCanvasLayout(value);
          break;
        case "captionStyle":
        ...
        case "customAspectRatio":
          ...
          break;
        case "activeInteractiveFilter":
          setActiveInteractiveFilter(value);
          break;
        ...
        case "filterTarget":
          ...
          break;
        case "isAutoFramingEnabled":
          setIsAutoFramingEnabled(value);
          break;
        case "isBeautifyEnabled":
          setIsBeautifyEnabled(value);
          break;
        case "isLowLightEnabled":
          setIsLowLightEnabled(value);
          break;
        case "isNeonEdgeEnabled":
          setIsNeonEdgeEnabled(value);
          break;
        case "neonIntensity":
        ...
        case "zoomSensitivity":
          ...
          break;
        case "trackingSpeed":
          setTrackingSpeed(value);
          break;
        case "isFaceTrackingEnabled":
          setIsFaceTrackingEnabled(value);
          break;
        case "canvasAspectRatio":
          setCanvasAspectRatio(value);
          break;
        case "videoFilter":
        ...
        case "backgroundImageUrl":
          ...
          break;
        case "blankCanvasColor":
          setBlankCanvasColor(value);
          break;

        default:
          console.warn(`updateSceneProperty: Unhandled key ${String(key)}`);
      }
    },
    [
      setAudioOn,
  ...
  useCanvasPaste({
    ...
    isDrawing, // Pass isDrawing to prevent paste capturing when drawing
  });
  ...
  // Real Layout Manager (replaces the stubbed one from props)
  ...
  const commonCallbacks = {
    ...
    handleGridAssetSelect: (id: string, asset: AssetResult) => {
      /* Reuse logic */
    },
    handleSectionCameraSettingsChange: (id: string, settings: any) => {
      /* Reimplement */
    },
      ...
      <MainCanvasArea
        ...
        globalCanvasProps={{
          ...
          onOverlayLayoutChange: (id, key, value) => {
            const updated = activeOverlays.map((o) =>
          ...
          onBrowserUrlChange: (id, url) =>
            setBrowserOverlays(
              browserOverlays.map((b) => (b.id === id ? { ...b, url } : b))
            ),
          ...
          onTextContentChange: (id, content) =>
            setTextOverlays(
              textOverlays.map((t) => (t.id === id ? { ...t, content } : t))
            ),
          ...
          onAddFile: (file) => {
            /* TODO: Implement global file add if needed, or rely on activeSceneProps */
          },

          // Generated
          selectedGeneratedId,
          setSelectedGeneratedId,
          onPreviewGenerated: (id, preview) =>
            setActiveOverlays(
              activeOverlays.map((o) => (o.id === id ? { ...o, preview } : o))
            ),
          onUpdateOverlayMetadata: (id, metadata) =>
            setActiveOverlays(
              activeOverlays.map((o) => (o.id === id ? { ...o, metadata } : o))
            ),
