...
export const CanvasContainer: React.FC<CanvasContainerProps> = ({
  ...
  const emptyGridPanels: EmptyGridPanelState[] = useMemo(
    () => activeScene?.emptyGridPanels || [],
    [activeScene?.emptyGridPanels],
  );
  const [selectedEmptyGridPanelId, setSelectedEmptyGridPanelId] = useState<
    string | null
  >(null);
  ...
  const handleAddEmptyGridPanel = useCallback(() => {
    ...
    updateActiveScene((prev: any) => {
      const currentPanels = prev?.emptyGridPanels || [];
      return {
        ...prev,
        emptyGridPanels: [...currentPanels, newPanel],
      };
    });
  ...
  const handleRemoveEmptyGridPanel = useCallback(
    (id: string) => {
      updateActiveScene((prev: any) => {
        const currentPanels = prev?.emptyGridPanels || [];
        return {
          ...prev,
          emptyGridPanels: currentPanels.filter((p: any) => p.id !== id),
        };
      });
      setSelectedEmptyGridPanelId((prevId) => (prevId === id ? null : prevId));
    },
    [updateActiveScene],
  );
  ...
  const handleEmptyGridPanelLayoutChange = useCallback(
    (id: string, layout: Partial<EmptyGridPanelState["layout"]>) => {
      updateActiveScene((prev: any) => {
        ...
        return {
          ...prev,
          emptyGridPanels: currentPanels.map((p: any) =>
  ...
  const commonCallbacks = useMemo(
    ...
    [
      ...
      updateActiveScene,
      layoutManager,
      bannerLogic.handleBannerTextStyleChange,
      bannerLogic.onBannerTextClose,
      activeOverlays,
      setActiveOverlays,
    ],
  );
  ...
  if (activeSceneProps?.sidebarProps) {
    (activeSceneProps.sidebarProps as any).onAddSocialBanner =
      bannerLogic.handleAddSocialBanner;
    (activeSceneProps.sidebarProps as any).onAddAnimatedBanner =
