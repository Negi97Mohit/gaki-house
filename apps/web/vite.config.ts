      ...
      server.middlewares.use('/api/youtube-live', async (req: any, res: any) => {
        try {
          ...
          const body = JSON.stringify({
              context: { client: { clientName: "WEB", clientVersion: "2.20240101.00.00", hl: lang, gl: region } },
              query: q, params: "EgJAAQ%3D%3D"
          });
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      "@caption-cam/engine": path.resolve(__dirname, "../../packages/engine/src"),
  ...
  server: {
    ...
    port: 5173,
    proxy: {
      '/api/kick': { target: 'https://kick.com/api/v1', changeOrigin: true, rewrite: (path) => path.replace(/^\/api\/kick/, '') },
      '/api/dlive': { target: 'https://graphigo.prd.dlive.tv', changeOrigin: true, rewrite: (path) => path.replace(/^\/api\/dlive/, '') }
    }
  }
});
