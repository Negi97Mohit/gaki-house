...
const AuthSheet = ({ open, onClose }: AuthSheetProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = messages[code] || "Something went wrong. Try again.";
      setError(message);
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      toast.success("Signed in with Google!", {
        description: `Welcome, ${credential.user.displayName || "there"}!`,
      });
      onClose();
    } catch (err: any) {
  ...
  const handleAppleSignIn = async () => {
    ...
    try {
      ...
      if (err?.code === "auth/popup-closed-by-user") return;
      // TODO: Apple Sign-In requires Service ID + key file configured in Firebase console.
      // Until that's done, this will throw an auth/operation-not-allowed error.
      const message =
        err?.code === "auth/operation-not-allowed"
          ? "Apple Sign-In is not yet configured. Use email or Google instead."
          : err?.message || "Apple sign-in failed. Try again.";
      setError(message);
          ...
          >
            ...
            <div className="space-y-2 mb-4">
              <button
                ...
                className="w-full h-11 rounded-2xl bg-white border border-foreground/15 flex items-center justify-center gap-2.5 font-medium text-sm text-foreground active:scale-[0.99] transition-transform shadow-float disabled:opacity-60"
              >
              ...
              <button
                ...
                className="w-full h-11 rounded-2xl bg-foreground text-background flex items-center justify-center gap-2.5 font-medium text-sm active:scale-[0.99] transition-transform shadow-float disabled:opacity-60"
              >
                <svg
                  className="h-4 w-4"
                  viewBox="0 0 24 24"
            ...
            <form onSubmit={handleSubmit} className="space-y-2.5">
              <div className="relative">
                ...
                <input
                  ...
                  disabled={loading}
                  className="w-full h-11 pl-10 pr-3 rounded-2xl bg-foreground/5 border border-foreground/10 text-sm placeholder:text-foreground/40 focus:outline-none focus:border-foreground/30 focus:bg-foreground/10 transition-colors disabled:opacity-60"
                />
              </div>
              <div className="relative">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full h-12 rounded-2xl bg-gradient-primary text-white font-semibold text-sm flex items-center justify-center gap-2 shadow-elevated active:scale-[0.99] transition-transform mt-1 disabled:opacity-70"
              >
                {loading ? (
